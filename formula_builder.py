from formula_manager import make_big_var, add_clauses, calculate_big_var_size
...
def remain_the_same(bottle_indicies, main_vars, step):
	if len(bottle_indicies) == 0:
		return ''
	formula = '()'
	for ind in bottle_indicies:
		formula = and_link(formula, are_numbers_equal(main_vars[ind][step + 1], main_vars[ind][step])) 
	return formula

def build_formula(steps_count, bottle_sizes, desired_number):
	...
	for step in range(steps_count - 1):
		...
		for i in range(bottles_count):
			local_formula = '()'
			local_formula = and_link(local_formula, is_number_equal_to_given_constant(main_vars[i][step + 1], bottle_sizes[i]))
		...
		for i in range(bottles_count):
			for j in range(bottles_count):
				...
				local_formula = and_link(local_formula, is_number_equal_to_given_constant(w, bottle_sizes[j]))
		...
		for i in range(bottles_count):
			for j in range(bottles_count):
				...
				remaining_bottles = list(range(min(i, j))) + list(range(min(i, j) + 1, max(i, j))) + list(range(max(i, j) + 1, bottles_count))
				local_formula = and_link(local_formula, remain_the_same(remaining_bottles, main_vars, step))
				local_formula = and_link(local_formula, is_number_equal_to_given_constant(main_vars[j][step + 1], bottle_sizes[j]))
				w, cur_var_number = make_big_var(big_var_size, cur_var_number)
